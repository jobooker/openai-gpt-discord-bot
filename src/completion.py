...
async def generate_chat_response(
    ...
    try:
        ...
        for message in messages:
            ...
            testFinalPrompt.append(newMessage)

        finalPrompt=[
                 {"role": "system", "content": f"You are {MY_BOT_NAME}: {BOT_INSTRUCTIONS}. Same conversations are: {MY_BOT_EXAMPLE_CONVOS}"},
                 {"role": "user", "content": f"{prompt.convo.render()}"}
             ]
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=testFinalPrompt
        )
        logger.info(testFinalPrompt)
